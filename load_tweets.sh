...
for file in $files; do
    ...
    time python3 -u load_tweets.py --db=postgresql://postgres:pass@localhost:1177/ "--inputs=$file"
    echo ''
done
...
for file in $files; do
    ...
    time unzip -p "$file" | sed 's/\\u0000//g' | psql postgresql://postgres:pass@localhost:1176/ -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
